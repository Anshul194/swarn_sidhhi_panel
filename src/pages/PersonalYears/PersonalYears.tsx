...
//     navigate("/personal-years/details", { state: { year } });
//   };
      ...
      {viewMode === "table" ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 py-4">
          {personalYears.map((year) => (
            <div
              key={year}
              className="border border-gray-400 rounded-lg p-8 flex items-center justify-center cursor-pointer hover:shadow-md transition text-2xl font-semibold text-gray-800 dark:text-white"
              onClick={() => handleNavigate(year)}
            >
              {year}
            </div>
