...
import { deleteTag, fetchTags, updateTag } from "../../../store/slices/tag";

const TagList: React.FC = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { tags, loading, error, pagination } = useSelector(
    (state: any) => state?.tag
  );
  ...
  const handleConfirmDelete = async () => {
    ...
    const token = localStorage.getItem("token") || "";
    const baseUrl = import.meta.env.VITE_BASE_URL || "";
    await dispatch(deleteTag(articleToDelete.id));
  ...
  const handleEdit = () => {
    dispatch(updateTag({ id: editTag.id, data: { name } }));
  };
  ...
  return (
    <>
        ...
        <div className="bg-white shadow rounded-lg overflow-x-auto dark:bg-gray-900">
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tbody className="bg-white divide-y divide-gray-100 dark:bg-gray-900 dark:divide-gray-800">
              {tags?.map((entrance: any, idx: number) => (
                <tr
      ...
      {editMode && (
        <div className="fixed top-0 h-screen w-full left-0  z-[9999] bg-black/10 backdrop-blur-[1px] p-4 flex justify-center items-center">
          <div className="w-1/3 p-6 flex flex-col gap-4 bg-white">
            <h1 className="text-2xl font-bold text-gray-800 dark:text-white/90">
              Edit List
            </h1>
