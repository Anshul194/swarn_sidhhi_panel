...
import { fetchYearPredictions } from "../../../../store/slices/yearPredictions";

const YearPredictionsList: React.FC = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { predictions, loading, error, pagination } = useSelector(
    (state: any) => state?.yearPredictions
  );
      ...
      <div className="bg-white shadow rounded-lg overflow-x-auto dark:bg-gray-900">
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          ...
          <tbody className="bg-white divide-y divide-gray-100 dark:bg-gray-900 dark:divide-gray-800">
            {predictions?.map((entrance: any, idx: number) => (
              <tr
                key={entrance?._id || idx}
                className="hover:bg-gray-50 dark:hover:bg-gray-800"
                onClick={() =>
                  navigate("/numerology/year-prediction/edit", {
                    state: { yearPredictionId: entrance?.id },
                  })
                }
              >
                ...
                <td className="px-6 py-4 text-sm font-medium text-gray-900 dark:text-white">
                  {entrance?.year_number || "-"}
                </td>
                <td className="px-6 py-4 text-sm font-medium text-gray-900 dark:text-white">
                  {entrance?.mulank_number || "-"}
                </td>
                <td className="px-6 py-4 text-sm font-medium text-gray-900 dark:text-white">
                  {entrance?.prediction.slice(0, 100)}{" "}
                  {entrance?.prediction.length > 100 ? "..." : ""}
                </td>
              </tr>
            ))}
          </tbody>
...
export default YearPredictionsList;
